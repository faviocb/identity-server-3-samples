...
namespace MyContacts.Controllers
{
    public class OAuth20Controller : Controller
    {
        ...
        private Dictionary<string, Tuple<string, string>> applicationRegistry = new Dictionary<string, Tuple<string, string>>();
        private static ConcurrentDictionary<Guid, Tuple<string, string, string>> codesIssued = new ConcurrentDictionary<Guid, Tuple<string,string, string>>();

        public OAuth20Controller()
        {
            // client id, client secret and domain
            applicationRegistry.Add("0123456789", new Tuple<string, string>("TXVtJ3MgdGhlIHdvcmQhISE=",  ConfigurationManager.AppSettings["UrlMyPromo-Host"]));
        }
        ...
        public ActionResult Index(AuthzCodeRequest request)
        {
            ...
            ViewBag.CipherText = String.Format("{0}|{1}", request.redirect_uri, request.scope).ToCipherText();
        ...
        public JsonResult Index(TokenRequest request)
        {
            ...
            if (isRequestValid)
            {
                JsonWebToken token = new JsonWebToken()
                {
                    ...
                    Issuer = ConfigurationManager.AppSettings["UrlMyContacts-OAuth20"],
                    Audience = ConfigurationManager.AppSettings["UrlMyPromo-Home"]
                };
                token.AddClaim(ClaimTypes.Name, grantData.Item1);
                token.AddClaim(ConfigurationManager.AppSettings["UrlMyContacts-OAuth20-Scope"], grantData.Item3);
